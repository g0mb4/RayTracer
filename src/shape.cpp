...
Plane::Plane(const Point & position, float size, const Vector & normal,
             float reflection, const Color & c):
    position(position),
    normal(normal), _size(size), reflection(reflection) {
    type = T_PLANE;
    color = c;
  ...
  */
    ...
    _t = Vector(1.0, 0, 0), _b = cross(_t, normal);
...
Ellipsoid::Ellipsoid(const Point & centre, const Point & radius,
                     float reflection, const Color & c):
    centre(centre),
    radius(radius), reflection(reflection) {
    type = T_ELLIPSOID;
    color = c;
...
bool Ellipsoid::intersect(Intersection & intersection) {
    ...
    double b = ((2 * localRay.origin.x * localRay.direction.x) /
                (radius.x * radius.x)) +
    ...
    double c =
        ((localRay.origin.x * localRay.origin.x) / (radius.x * radius.x)) +
        ((localRay.origin.y * localRay.origin.y) / (radius.y * radius.y)) +
        ((localRay.origin.z * localRay.origin.z) / (radius.z * radius.z)) - 1;
...
bool Ellipsoid::isInside(const Point p) {
    double a = ((p.x - centre.x) / radius.x) * ((p.x - centre.x) / radius.x);
    double b = ((p.y - centre.y) / radius.y) * ((p.y - centre.y) / radius.y);
    double c = ((p.z - centre.z) / radius.z) * ((p.z - centre.z) / radius.z);
...
void Cube::draw(void) {
    ...
    // front face
    glVertex3f(centre.x - halfSideLength, centre.y + halfSideLength,
               centre.z + halfSideLength); // top left
    glVertex3f(centre.x + halfSideLength, centre.y + halfSideLength,
               centre.z + halfSideLength); // top right
    glVertex3f(centre.x + halfSideLength, centre.y - halfSideLength,
               centre.z + halfSideLength); // bottom right
    glVertex3f(centre.x - halfSideLength, centre.y - halfSideLength,
               centre.z + halfSideLength); // bottom left
